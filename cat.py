...
for b in [5, 10, 25]:
    if num_hash % b == 0:
        ...
        p, r, f = prf(tp, fp, fn)
        bucket_rows.append([b, len(cand), p, r, f])
...
query_relevance = {
    "information retrieval" : {0, 1, 2, 3},
    "query expansion" : {6, 7, 8},
    "search engines" : {3, 4, 5},
    "duplicate detection" : {9, 10, 11}
}
...
plt.xlabel("Documents")
plt.ylabel("Documents")
plt.suptitle("Similarity Comparison : MinHash vs Jaccard")
plt.show()
...
plt.title("Precision Recall Fscore vs Bucket Size")
...
plt.title("Signature Compression Ratio vs Accuracy")
...
plt.title("MAP Before vs After Rocchio Reweighting")
plt.xlabel("Reweighting Settings")
plt.ylabel("MAP")
plt.legend()
plt.show()

plt.figure()
plt.bar(labels, map_df["Percent Change"])
plt.title("Percent Change in MAP for Different Reweighting")
plt.xlabel("Reweighting Settings")
